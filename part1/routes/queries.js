...
router.get('/walkers/summary', async (req, res) => {
    const [rows] = await db.query(`
        SELECT u.username AS walker_username, COUNT(rate.rating_id) AS total_ratings, AVG(rate.rating) AS average_rating, COUNT(requests.request_id WHERE requests.status = 'completed') AS completed_walks
        FROM Users u
